...
" >> 常用命令提示 -------
  ...
  function! ZCommandHelp()
    ...
    echo '    翻譯器：'
    echo '        翻譯目標語言：'
    echo '            @TT[en|ja] : 中文/英文/日文'
    echo '        @TT : 在命令行中顯示翻譯    @TW : 在浮動框中顯示翻譯'
    echo '        @TR : 用翻譯替換文本        @TX : 翻譯剪貼板中的文本'
    echo ' '
    echo '    命令行著色：'
    echo '        @rcc : 預設/著色切換'
    echo ' '
    ...
    echo ' '
    echo '    Neo: Coc 主機語言服務器 和 Nodejs 擴展工具：'
    echo '        文件說明：'
    echo '            K     : 顯示文件說明'
    echo '            <C-b> : 說明窗上捲            <C-f> : 說明窗下捲'
    echo '        導航至診斷位置：'
    echo '            @cg[ : 移動至上一個診斷位置'
    echo '            @cg] : 移動至上一個診斷位置'
    echo '        導航至代碼位置：'
    echo '            @cgd : 導航至代碼位置'
    echo '            @cgi : 宣告變量參考來源'
    echo '            @cgy : 未知 (type-definition [類型定義])'
    echo '            @cgr : 未知 (implementation [履行])'
